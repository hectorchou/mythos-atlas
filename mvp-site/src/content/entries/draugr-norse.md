---
id: draugr-norse
name_primary: Draugr
name_original: Draugr（古诺尔斯语）
name_aliases: [Aptrganga, Haugbúi, Revenant]
name_translations:
  zh: 德拉乌格
  en: Draugr
  de: Draug
  is: Draugur

hero_image: /images/entries/draugr-norse.jpg
hero_image_alt: "Draugr · 北欧尸鬼"
hero_image_credit: "AI generated · Nordic saga illustration style"

culture_path: 西欧/北欧/维京民俗/亡灵
entity_type: creature
era: 13-14世纪萨迦文献系统记载，口传更早
geo_region: 冰岛、挪威、瑞典、设得兰群岛

summary: 北欧萨迦中自坟墓复生的尸鬼，身具超常力量与巫术能力，守护墓中财宝并侵扰生者。
attributes: [尸身不腐, 超常力量, 可变大缩小, 操控天气, 惧斩首与火化, 守护宝藏]

primary_sources:
  - type: book
    title: "Eyrbyggja Saga"
    author: "佚名（13世纪冰岛萨迦）"
    year: 1260
    language: is
    location: 第34章至第44章（Þórólfr haltfótr与Víga-Styrr的draugr情节）
    url: https://sagadb.org/eyrbyggja_saga
    access: open
    note: "冰岛萨迦中最著名的draugr叙事，包含尸鬼复活、压迫生者、最终被火化解的完整情节弧"
  - type: book
    title: "Grettis Saga"
    author: "佚名（14世纪冰岛萨迦）"
    year: 1320
    language: is
    location: 第18章至第21章（Grettir与Glámr之战）
    url: https://sagadb.org/grettis_saga
    access: open
    note: "最经典的draugr对决——英雄Grettir与尸鬼Glámr的搏斗，直接影响了后世僵尸文学"
  - type: book
    title: "Laxdæla Saga"
    author: "佚名（13世纪冰岛萨迦）"
    year: 1250
    language: is
    url: https://sagadb.org/laxdaela_saga
    access: open

secondary_sources:
  - type: paper
    title: "The Undead in the Old Norse Sagas: A Study of the Draugr Motif"
    author: "Armann Jakobsson"
    year: 2009
    language: en
    url: https://doi.org/10.1484/J.VIATOR.1.100120
    access: paywall
    note: "当代北欧中世纪研究权威对draugr母题的系统性分析"
  - type: book
    title: "The Norse Myths: A Guide to the Gods and Heroes"
    author: "Carolyne Larrington"
    year: 2017
    language: en
    note: "牛津大学北欧文学教授，权威入门"

confidence: attested
first_recorded: "13世纪（冰岛萨迦抄本）"

parallel_motifs:
  - entry_id: shuigui-china
    relation: "同为无法安息的亡灵——draugr因宝藏贪欲或生前恶行而复生，水鬼因溺亡不得投胎，均属不得善终者化为厉鬼跨文化母题"
  - entry_id: xibalba-maya
    relation: "北欧与玛雅对死后世界的不同建构——draugr留在现世侵扰活人，Xibalba则是系统化的死后审判与考验空间"

curator: hector
review_status: draft
llm_assisted: false
created_at: "2026-07-04"
updated_at: "2026-07-04"
---

## 描述

Draugr 是北欧萨迦传统中最具辨识度的超自然存在之一。不同于现代流行文化中的"丧尸"或"吸血鬼"，draugr 是**保留肉体实体的复生者**——他们以已腐烂或干化的尸身行动，拥有远超生者的力量，可以随意改变身形大小，操控天气，甚至通过巫术（seiðr）施加诅咒。

## 经典叙事模式

萨迦中的 draugr 故事通常遵循一个固定结构：
1. **某人死后不安葬或葬于特定地点**
2. **生者遭遇骚扰**：家畜死亡、屋顶被破坏、夜间有巨物压身
3. **英雄介入**：通常是具有超常勇气与力量的维京人
4. **对决**：英雄进入墓冢（haugr）与draugr搏斗
5. **解决方式**：斩首并焚烧，或将尸骨抛入远海

## 文化意义

Draugr 不仅是民间恐怖元素，更是冰岛萨迦作者用来探讨**荣誉、遗产与家族诅咒**的叙事工具。Glámr 在《Grettis Saga》中对 Grettir 的诅咒，是北欧文学中关于"厄运与勇气"的最深刻表达之一。

## 现代影响

Draugr 直接影响了 J.R.R. Tolkien 的《指环王》中的"戒灵"概念，以及现代流行文化中"尸鬼"形象的塑造。游戏《上古卷轴》系列中的"Draugr"直接借用了此名称。
